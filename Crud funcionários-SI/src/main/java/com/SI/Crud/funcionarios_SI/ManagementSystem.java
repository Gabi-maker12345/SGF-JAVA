...
public class ManagementSystem {
            ...
            try {
                ...
                switch (option) {
                    ...
                    case "6":
                        ...
                        return;
                    default:
                        System.out.println("Erro: Opção inválida! Digite apenas números de 1 a 6.");
                }
            } catch (Exception e) {
    ...
    private static void showMenu() {
        ...
        System.out.println("==================================================");
        System.out.println("1. Criar Conta de Funcionário");
        System.out.println("2. Ver Detalhes do Funcionário (Visão Individual)");
        System.out.println("3. Alterar Salário (Validação de Mínimo Legal)");
        System.out.println("4. Excluir Conta de Funcionário");
        System.out.println("5. Consultar Painel de Auditoria e Logs");
    ...
    private static void createEmployeeAccount() {
        try {
            System.out.print("Digite o ID para o novo funcionário: ");
            ...
            if (database.containsKey(id)) {
                System.out.println("Erro: Já existe um funcionário com este ID.");
                return;
            }
            ...
            System.out.print("Digite o Salário Base em Kz (Mínimo: 100.000,00 Kz): ");
            String salaryInput = scanner.nextLine().trim();

            String cleanValue = salaryInput.replace(".", "").replace(",", ".");
            double salary = Double.parseDouble(cleanValue);
            ...
            AuditService.logAction(
                currentOperator, 
                "CREATE_ACCOUNT", 
                String.format("Nova conta criada - ID: %s, Nome: %s, Dept: %s, Salario: %,.2f Kz", id, name, department, salary)
            );
            System.out.println("Conta de funcionário criada com sucesso!");

        } catch (NumberFormatException e) {
            System.out.println("Erro: Digite um valor de salário válido.");
            AuditService.logAction(currentOperator, "CREATE_ACCOUNT_FAILURE", "Erro de digitação numérica no salário.");
        } catch (IllegalArgumentException e) {
            System.out.println("Erro: " + e.getMessage());
            AuditService.logAction(currentOperator, "CREATE_ACCOUNT_FAILURE", "Violação de regras de validação.");
        } catch (Exception e) {
    ...
    private static void deleteEmployeeAccount() {
        try {
            ...
            Employee emp = database.remove(id);
            AuditService.logAction(
                currentOperator, 
                "DELETE_ACCOUNT", 
                String.format("Conta removida definitivamente - ID: %s, Nome: %s", id, emp.getName())
            );
            System.out.println("Conta de funcionário removida com sucesso do sistema.");

        } catch (Exception e) {
            System.out.println("Erro ao tentar excluir a conta.");
        }
    }
